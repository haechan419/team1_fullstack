...
export default function ReportAnalyticsPage() {
    ...
    const FILTERS = useMemo(
        () => [
            {key: "period", label: "Period", type: "single", options: ["2025-01", "2025-02", "2025-03", "2025-04"]},
    ...
    useEffect(() => {
        if (role === "EMPLOYEE") {
            ...
            // ADMIN 기본값
            setSelected((prev) => ({...prev, scope: prev.scope?.length ? prev.scope : ["Department"]}));
        }
    }, [role]);
    ...
    const activeFilter = activeKey ? FILTERS.find((f) => f.key === activeKey) : null;

    const filteredOptions = useMemo(() => {
        if (!activeFilter) return [];
        const q = (filterSearch[activeFilter.key] || "").trim().toLowerCase();
        if (!q) return activeFilter.options;
        return activeFilter.options.filter((opt) => String(opt).toLowerCase().includes(q));
    ...
    const previewData = useMemo(() => {
        ...
        const report =
            visibleReportTypes.find((r) => r.id === applied.reportTypeId) || visibleReportTypes[0];
            ...
            <div className="report-page">
                                    ...
                                    <div className="dropdown-options">
                                        {filteredOptions.length === 0 ? (
                                            ...
                                            filteredOptions.map((opt) => {
                                                const checked = (selected[activeKey] || []).includes(opt);
                                                const inputType = activeFilter.type === "single" ? "radio" : "checkbox";
                                                const disabled = role === "EMPLOYEE" && activeKey === "scope";

                                                return (
                                                    <label key={opt}
                                                           className={`dropdown-item ${disabled ? "is-disabled" : ""}`}>
                                                        <input
                                                            type={inputType}
                                                            name={`dd-${activeKey}`}
                                                            checked={checked}
                                                            disabled={disabled}
                                                            onChange={() => selectOption(activeKey, activeFilter.type, opt)}
                                                        />
                ...
                <section className="section">
                    ...
                    <div className="panel preview-box">
                        <div className={`doc-viewer ${previewData.isReady ? "is-ready" : "is-empty"}`}>
                            {!previewData.isReady ? (
                                <div className="doc-empty">
                                    <div>
                                        <div className="doc-empty-title">Report Type is not applied</div>
                                            ...
                                            <div className="pdf-page">
                                                ...
                                                <div className="pdf-body">
                                                    ...
                                                    <div className="pdf-paragraph">
                                                        This document is a preview representation of the selected report
                                                        settings. It simulates
                                                        margins and typography similar to a generated PDF output.
                                                    </div>
                                                </div>
